...
| job | pid | command | log |
|---|---|---|---|
| template sweep (corrected): all discrete parameter tuples of the 14Handoff.md §7 construction template at p = 13, 19, 31, 37, 43 — looking for a tuple that works at every prime | 17326 | `PYTHONPATH=code .venv/bin/python code/mult_template.py 13 19 31 37 43` | `results/log_mult_template2.txt` |

Success criterion: a parameter tuple (phi, gam, del, eps, miss, one) that
verifies at *all* of 13, 19, 31, 37, 43 — after normalising by the label
ambiguity j ↦ λj (λ ∈ {1,2}) on cubic-class indices, the only freedom in
labelling K_1 vs K_2. Use `code/mult_intersect.py <log>` to intersect.
So far: 72 verified tuples at each of p = 13 and 19.

**Do not use the pre-correction log** `results/log_mult_template.txt`: that
run solved the fixed half with the generic `fixed_part`, which times out at
